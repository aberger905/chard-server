...
class ArticleService {
  ...
  private createPrompt = (input: ArticleInput): string => {
    ...
    if (articleType == 'featured') {
      ...
      prompt = `The following prompt contains information inserted by our users, indicated with <>. For example, <this is a user response>. You are now a news journalist writing a story about someone. Please write a 1000-word news article based on the input provided. Focus on the individual: <${firstName} ${lastName}>, using pronouns <${pronouns}>. The subject of this article is: <${subject}>. Relevant information about the individual, as provided by the user, is: <${story}>. That information was given by <${narrator}>. Please do not fabricate any details. You may discuss the broader subject matter. Include any quotes or statements as provided by the user, relevant to the individual’s story. I would like the response in JSON format. The JSON object should have two keys: 'title' and 'content'. The 'title' key should have a string value representing the title of the article. The 'content' key should be an array, with each element being a string that represents a section of the article. Each section could be a paragraph, a sentence, or a significant quote. Please ensure all strings are correctly escaped for JSON and formatted as single-line strings within the array to comply with JSON standards.`
    }
